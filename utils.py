...
def get_data(path):
    ...
    data=pd.read_csv(os.path.join(path,'data/dashboard_data.csv'),index_col=0).drop(columns=['TARGET'])
    return data

def get_info(SK_ID_CURR_user,data):
    return data[data.SK_ID_CURR==SK_ID_CURR_user].drop(['index','SK_ID_CURR'],axis=1).dropna(axis='columns')

def show_logo():
    return Image.open(os.path.join(path,'img/LOGO.png'))

def get_model_and_score(info):
        model = pickle.load(open(os.path.join(path,'pickle/model_pkl2'), 'rb'))
        seuil=0.31

        # import des données
        X_test=pd.read_csv(os.path.join(path,'data/test.csv'),index_col=0)
...
def convertUpperAndLowerBoundAndThreshold(value,oldMin,oldMax,oldThreshold,newMin,newMax,newThreshold):
...
def gauge_chart(score, minScore, maxScore, threshold):
    ...
    localThreshold=0
    convertedScore = convertUpperAndLowerBoundAndThreshold(value=score,
                                                            oldMin=minScore,
                                                            oldMax=maxScore,
...
def get_global_feat_imp(model,X_test):
    ...
    top_features=feat_importance.sort_values(by='Global Feature Importance',ascending=False).head(10)
...
def get_local_feat_imp(model,X_test,info):
    ...
    local=pd.DataFrame({'Local Feature Importance':serieValues,'absValues':serieAbsValues,'positive':serieSignPositive,
                'color':map(lambda x: 'red' if x else 'blue', serieSignPositive)}).sort_values(
                by='absValues',ascending=False).iloc[:12,:].drop('absValues', axis=1)
...
def plot_global(top_features):
    ...
    sns.barplot(data=top_features,x='Global Feature Importance',y='Feature Name',palette='crest')
    return fig
